...
parser.add_argument("--savename", default="./Alex_Nat", type=str)
...
parser.add_argument("--index_path", default="Alex_Nat", type=str)
parser.add_argument("--emb_path", default=r"./refer/Alex_Nat/uni", type=str)
...
for fold in range(4):
    ...
    temp_arg = temp_arg(args.size, args.numk, args.mdim, args.emb_path + f"/{fold}/" + args.index_path, args.emb_path,
                        args.data)
    train_dataset = AlexDataset(KFOLD[fold][0], None, None, temp_arg, train=True)
    ...
    for iid in range(len(KFOLD[fold][0]) + len(KFOLD[fold][1])):
        ...
        dataset = AlexDataset([iid], None, None, temp_arg, train=False)
